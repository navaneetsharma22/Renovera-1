...
export function SearchAndFilters({ 
  ...
  return (
    <div className={cn("sticky top-[72px] z-40 bg-[#FAFAFA]/95 backdrop-blur-md border-b border-[#E4E4E7]", className)}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-[1400px]">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 py-4 md:py-6">
          ...
          <div className="hidden md:flex flex-wrap items-center gap-3 ml-auto">
            ...
            >
              {projectsPageData.filters.categories.map(cat => (
            ...
            <select 
              ...
              className="bg-transparent border-none text-xs font-bold uppercase tracking-widest text-[#52525B] focus:outline-none focus:ring-0 cursor-pointer hover:text-[#09090B] transition-colors"
            >
              {projectsPageData.filters.locations.map(loc => (
            ...
            <select 
              ...
              className="bg-transparent border-none text-xs font-bold uppercase tracking-widest text-[#52525B] focus:outline-none focus:ring-0 cursor-pointer hover:text-[#09090B] transition-colors"
            >
              {projectsPageData.filters.styles.map(style => (
        ...
        {isMobileFiltersOpen && (
          <div className="md:hidden py-4 border-t border-[#E4E4E7] flex flex-col gap-4">
             <div className="flex flex-col gap-2">
              ...
              >
                {projectsPageData.filters.categories.map(cat => <option key={cat} value={cat}>{cat}</option>)}
              </select>
            </div>
            
            <div className="flex flex-col gap-2">
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#C5A059]">Location</span>
              <select 
                value={activeLocation}
                onChange={(e) => setActiveLocation(e.target.value)}
                className="w-full bg-white border border-[#E4E4E7] rounded-md p-2 text-sm"
              >
                {projectsPageData.filters.locations.map(loc => <option key={loc} value={loc}>{loc}</option>)}
              </select>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <select 
                ...
                className="w-full bg-white border border-[#E4E4E7] rounded-md p-2 text-sm"
              >
                {projectsPageData.filters.styles.map(style => <option key={style} value={style}>{style}</option>)}
              </select>
            </div>
          </div>
        )}
