...
export function AboutHero({ className }) {
  const sectionRef = useRef(null);
  const bgRef = useRef(null);
  const contentRef = useRef(null);

  useGSAP(() => {
    // Cinematic background scale
    gsap.fromTo(bgRef.current,
      { scale: 1.05 },
      { scale: 1, duration: 2, ease: "power2.out" }
    );

    // Fade up content staggered
    ...
    <section ref={sectionRef} className={cn("relative min-h-[90vh] flex items-center justify-center overflow-hidden pt-24", className)}>
      ...
      <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center">
        <div ref={contentRef} className="max-w-[800px] mx-auto flex flex-col items-center">
          ...
          <h1 className="text-5xl md:text-6xl lg:text-8xl font-bold font-heading tracking-tight leading-[1.1] text-white mb-8 whitespace-pre-line">
